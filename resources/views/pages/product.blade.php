    ...
    <section class="bread-crumb">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('products_page') }}">Product</a></li>
        ...
        <section class="section-product">
            <div class="section-header">
                ...
                <div class="section-sub-title">
                    <div class="sku-code">Product code: <i>{{ $data['product']->sku_code }}</i></div>
                    <div class="start-vote">{!! Helper::get_start_vote($data['product']->rate) !!}</div>
                    <div class="rate-link" onclick="scrollToxx();"><span>Product Reviews</span></div>
                </div>
            </div>
                                    ...
                                    @foreach ($data['product_details'] as $key => $product)
                                        ...
                                        @if (
                                            ...
                                            <div class="sale-price">
                                                {{ number_format($product->promotion_price, 0, ',', '.') }} <span>$</span>
                                            </div>
                                            <div class="promotion-price">
                                                <div class="old-price">Giá cũ:
                                                    <del>{{ number_format($product->sale_price, 0, ',', '.') }}</del>
                                                    <span>$</span></div>
                                                <div class="save-price">Giảm:
                                                    <span>{{ number_format($product->sale_price - $product->promotion_price, 0, ',', '.') }}</span>
                                                    <span>$</span></div>
                                            </div>
                                        @else
                                            <div class="sale-price">{{ number_format($product->sale_price, 0, ',', '.') }}
                                                <span>$</span></div>
                                        @endif
                                        @if ($product->quantity > 0)
                                            <div class="status">Status: <span style="color: #1a2;">Stocking</span></div>
                                        @else
                                            <div class="status">Status: <span style="color: #f30;">Out of stock</span></div>
                                        @endif
                                </div>
                                @endforeach
                            </div>
                                        ...
                                        @foreach ($data['product_details'] as $key => $product)
                                            <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                                                <div class="color-inner {{ $key == 0 ? 'active' : '' }}"
                                                    product-id="{{ $product->id }}" data-key="{{ $key }}"
                                                    ...
                                                    <div class="image-check"><img
                                                            src="{{ asset('images/select-pro.png') }}"></div>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
                                            <button type="submit" class="btn btn-lg btn-gray"><i
                                                    class="far fa-money-bill-alt"></i> Buy now</button>
                                        </div>
                                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
                                            <div class="form-center">
                                                ...
                                                <input type="text" class="qty input-text" id="qty" name="quantity"
                                                    ...
                                                    min="0" max="{{ $data['product_details'][0]->quantity }}"
                                                    disabled>
                                        ...
                                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
                                            <button type="button" data-role="addtocart"
                                                ...
                                                data-url="{{ route('add_cart') }}"><span class="txt-main"><i
                                                        class="fa fa-cart-arrow-down padding-right-10"></i>
                                                    Cart</span></button>
                                        </div>
                                    </div>
                                </form>
                ...
                <div class="tab-description">
                    <div class="tab-header">
                        <ul class="nav nav-tabs nav-tab-custom">
                            <li class="active"><a data-toggle="tab" href="#description">Describe</a></li>
                            <li><a data-toggle="tab" href="#vote">Review and evaluation</a></li>
                        </ul>
                    </div>
                    <div class="tab-content">
                        <div id="description" class="tab-pane fade in active">
                            <div class="content-description">
                                ...
                                @else
                                    <p class="text-center"><strong>Updating ...</strong></p>
                                @endif
                            </div>
                            <div class="loadmore" style="display: none;"><a>Read more <i
                                        class="fas fa-angle-down"></i></a></div>
                            <div class="hidemore" style="display: none;"><a>Shorten <i class="fas fa-angle-up"></i></a>
                            </div>
                        </div>
                            ...
                            <div class="content-vote">
                                @if (Auth::check())
                                    <div class="section-rating">
                                        <div class="rating-title">Product Reviews</div>
                                        <div class="rating-content">
                                            ...
                                            <div class="rating-form">
                                                <form action="{{ route('add_vote') }}" method="POST"
                                                    ...
                                                    <button type="submit" class="btn btn-default">Submit a
                                                        review</button>
                                                </form>
                                            </div>
                                        </div>
                                    ...
                                    <div class="show-rate-content">
                                        <div class="total-rate">
                                            ...
                                            <div class="total-rate-right">
                                                ...
                                                <div class="total-user">{{ $data['product_votes']->count() }} <i
                                                    ...
                                                    <div class="vote-content">
                                                        ...
                                                        <div class="vote-content-right">
                                                            ...
                                                            <div class="vote-start">
                                                                ...
                                                                <div class="date">
                                                                    {{ date_format($vote->created_at, 'd/m/Y') }}</div>
                                                            </div>
                                        ...
                                        @else
                                            <p class="text-center"><strong>There are no user reviews yet. Let us know your
                                                    opinion.</strong></p>
                                        @endif
                                    </div>
                                </div>
            ...
            <div class="col-lg-3 col-md-4">
                <div class="infomation-detail">
                    ...
                    <div class="infomation-content">
                        <ul>
                            <li><label>Screen:</label>{{ $data['product']->monitor ?: 'Đang Cập Nhật...' }}</li>
                            <li><label>Front camera:</label>{{ $data['product']->front_camera ?: 'Đang Cập Nhật...' }}</li>
                            <li><label>Rear Camera:</label>{{ $data['product']->rear_camera ?: 'Đang Cập Nhật...' }}</li>
                            <li><label>Ram:</label>{{ $data['product']->RAM != 0 ? $data['product']->RAM . ' GB' : 'Đang Cập Nhật...' }}
                            </li>
                            <li><label>Internal
                                    memory:</label>{{ $data['product']->ROM != 0 ? $data['product']->ROM . ' GB' : 'Đang Cập Nhật...' }}
                            </li>
                            <li><label>CPU:</label>{{ $data['product']->CPU ?: 'Đang Cập Nhật...' }}</li>
                            <li><label>GPU:</label>{{ $data['product']->GPU ?: 'Đang Cập Nhật...' }}</li>
                            <li><label>Battery capacity:</label>{{ $data['product']->pin ?: 'Đang Cập Nhật...' }}</li>
                            <li><label>Operating system:</label>{{ $data['product']->OS ?: 'Đang Cập Nhật...' }}</li>
                        </ul>
                    </div>
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Detailed Specifications</h4>
                </div>
