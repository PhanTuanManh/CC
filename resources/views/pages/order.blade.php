        ...
        <section class="section-order">
            <div class="section-header">
                <div class="section-header-left">
                    <h2 class="section-title">{{ $data['order']->order_code }} <span>( {{ $qty }} Product)</span>
                    </h2>
                </div>
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <div class="order-info-left">
                                        ...
                                        <div class="order-info-content">
                                            ...
                                            <div><span>Phone number</span> <span>{{ $data['order']->user->phone }}</span>
                                            </div>
                                            <div><span>Address</span> <span>{{ $data['order']->user->address }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-12 col-sm-12 col-xs-12">
                                    <div class="order-info-center">
                                        ...
                                        <div class="order-info-content">
                                            ...
                                            <div><span>Payment methods</span>
                                                <span>{{ $data['order']->payment_method->name }}</span></div>
                                            <div><span>Quantity</span> <span>{{ $qty }} Product</span></div>
                                            <div><span>Unit price</span> <span
                                                    style="color: #f30;">{{ number_format($price, 0, ',', '.') }}$</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        @foreach ($data['order']->order_details as $key => $order_detail)
                                            <tr>
                                                ...
                                                <td class="text-center"><a
                                                        href="{{ route('product_page', ['id' => $order_detail->product_detail->product->id]) }}"
                                                        title="{{ $order_detail->product_detail->product->name }}">{{ $order_detail->product_detail->product->sku_code }}</a>
                                                </td>
                                                <td class="text-center">{{ $order_detail->product_detail->product->name }}
                                                </td>
